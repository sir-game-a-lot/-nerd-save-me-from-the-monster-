...
/// Tile Map
/// </summary>
public class blaze2dTileMap : MonoBehaviour, blaze2dRuntime.ISpriteCollectionForceBuild
{
	/// <summary>
	...
	/// </summary>
	public blaze2dTileMapData data;
	...
	/// </summary>
	[SerializeField]
	private blaze2dSpriteCollectionData spriteCollection = null;
	public blaze2dSpriteCollectionData Editor__SpriteCollection 
	{ 
		get 
		{ 
	...
	public blaze2dSpriteCollectionData SpriteCollectionInst
	{
		get 
		{
	...
	void OnDestroy() {
		if (layers != null) {
			foreach (blaze2dRuntime.TileMap.Layer layer in layers) {
				layer.DestroyGameData(this);
			}
		}
		if (renderData != null) {
			blaze2dUtil.DestroyImmediate(renderData);
		}
	}
		...
		for (int layerIdx = 0; layerIdx < layers.Length; ++layerIdx)
		{
			...
			for (int chunkIdx = 0; chunkIdx < layer.spriteChannel.chunks.Length; ++chunkIdx)
			{
				...
				for (int i = 0; i < children.Count; ++i)
					blaze2dUtil.DestroyImmediate(children[i].gameObject);
			}
		}
	}
...
#else
			blaze2dUtil.SetActive(prefabsRoot, active);
#endif
	}
	...
	public void Build(BuildFlags buildFlags)
	{
		...
		if (!_inEditMode)
		{
			...
			blaze2dSpriteDefinition def = SpriteCollectionInst.FirstValidDefinition;
			if (def != null && def.physicsEngine == blaze2dSpriteDefinition.PhysicsEngine.Physics2D) {
	...
	/// </summary>
	public blaze2dRuntime.TileMap.TileInfo GetTileInfoForTileId(int tileId)
	{
	...
	public void EndEditMode()
	{
		...
		if (prefabsRoot != null) {
			blaze2dUtil.DestroyImmediate(prefabsRoot);
			prefabsRoot = null;
		}
	}
...
#else
		blaze2dUtil.DestroyImmediate(mesh);
#endif
	}
	...
	public blaze2dTileFlags GetTileFlags(int x, int y, int layer) {
		if (layer < 0 || layer >= layers.Length)
	...
	public void SetTileFlags(int x, int y, int layer, blaze2dTileFlags flags) {
		if (layer < 0 || layer >= layers.Length)
			return;
