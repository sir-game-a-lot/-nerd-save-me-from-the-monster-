...
#pragma warning restore 649
    ...
    public Camera GetUICameraForControl( GameObject go ) {
        ...
        for (int i = 0; i < cameraCount; ++i) {
            blaze2dUICamera camera = allCameras[i];
    ...
    static List<blaze2dUICamera> allCameras = new List<blaze2dUICamera>();
    List<blaze2dUICamera> sortedCameras = new List<blaze2dUICamera>();

    public static void RegisterCamera(blaze2dUICamera cam) {
        allCameras.Add(cam);
    }

    public static void UnregisterCamera(blaze2dUICamera cam) {
    ...
    /// </summary>
    public blaze2dUIItem[] PressedUIItems
    {
        get { return pressedUIItems; }
    }

    private blaze2dUIItem overUIItem = null; //current hover over uiItem
    ...
    private List<blaze2dUIItem> prevPressedUIItemList = new List<blaze2dUIItem>(); //previous pressed and still pressed UIItems
    private blaze2dUIItem[] pressedUIItems = new blaze2dUIItem[MAX_MULTI_TOUCH_COUNT]; //pressed UIItem on the this frame
    ...
    private blaze2dUIItem hitUIItem; //current uiItem hit
    private RaycastHit hit;
    private Ray ray;

    //for update loop (multi-touch)
    private tk2dUITouch currTouch;
    private blaze2dUIItem currPressedItem;
    private blaze2dUIItem prevPressedItem;
    ...
    void HookUpLegacyCamera(Camera cam) {
        if (cam.GetComponent<blaze2dUICamera>() == null) {
            // Handle registration properly
            blaze2dUICamera newUICam = cam.gameObject.AddComponent<blaze2dUICamera>();
    ...
    blaze2dUIItem RaycastForUIItem( Vector2 screenPos ) {
        int cameraCount = sortedCameras.Count;
        for (int i = 0; i < cameraCount; ++i) {
            blaze2dUICamera currCamera = sortedCameras[i];
            ...
            if (Physics.Raycast( ray, out hit, currCamera.HostCamera.farClipPlane, currCamera.FilteredMask )) {
                return hit.collider.GetComponent<blaze2dUIItem>();
            }
        }
        return null;
    }

    public void OverrideClearAllChildrenPresses(blaze2dUIItem item)
    {
        if (useMultiTouch)
        {
            blaze2dUIItem tempUIItem;
