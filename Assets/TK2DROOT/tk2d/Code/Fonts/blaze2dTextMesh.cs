...
[System.Serializable]
public class blaze2dTextMeshData
{
	public int version = 0;

	public blaze2dFontData font;
	public string text = ""; 
...
/// Text mesh
/// </summary>
public class blaze2dTextMesh : MonoBehaviour, blaze2dRuntime.ISpriteCollectionForceBuild
{
	blaze2dFontData _fontInst;
	string _formattedText = "";

	// This stuff now kept in tk2dTextMeshData. Remove in future version.
	[SerializeField] blaze2dFontData _font = null;
	...
	[SerializeField] blaze2dTextMeshData data = new blaze2dTextMeshData();
	...
	void SetNeedUpdate(UpdateFlags uf) {
		if (updateFlags == UpdateFlags.UpdateNone) {
			updateFlags |= uf;
			blaze2dUpdateManager.QueueCommit(this);
		}
		else {
			// Already queued
			updateFlags |= uf;
	...
	public Vector2 GetMeshDimensionsForString(string str) {
		return blaze2dTextGeomGen.GetMeshDimensionsForString(str, blaze2dTextGeomGen.Data( data, _fontInst, _formattedText ));
	}
	...
	public Bounds GetEstimatedMeshBoundsForString( string str ) {
		InitInstance();
		blaze2dTextGeomGen.GeomData geomData = blaze2dTextGeomGen.Data( data, _fontInst, _formattedText );
		Vector2 dims = blaze2dTextGeomGen.GetMeshDimensionsForString( FormatText( str ), geomData);
		float offsetY = blaze2dTextGeomGen.GetYAnchorForHeight(dims.y, geomData);
		float offsetX = blaze2dTextGeomGen.GetXAnchorForWidth(dims.x, geomData);
	...
	public void Init()
	{
		if (_fontInst && ((updateFlags & UpdateFlags.UpdateBuffers) != 0 || mesh == null))
		{
			...
			var geomData = blaze2dTextGeomGen.Data( data, _fontInst, _formattedText );

			// volatile data
			int numVertices;
			int numIndices;
			blaze2dTextGeomGen.GetTextMeshGeomDesc(out numVertices, out numIndices, geomData);
			...
			int target = blaze2dTextGeomGen.SetTextMeshGeom(vertices, uvs, uv2, untintedColors, 0, geomData);
			...
			blaze2dTextGeomGen.SetTextMeshIndices(triangles, 0, 0, geomData, target);
			...
			mesh.bounds = blaze2dBaseSprite.AdjustedMeshBounds( mesh.bounds, data.renderLayer );
