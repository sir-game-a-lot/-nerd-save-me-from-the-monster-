...
[AddComponentMenu("blaze2D/Sprite/tk2dSlicedSprite")]
...
/// </summary>
public class blaze2dSlicedSprite : blaze2dBaseSprite
{
	Mesh mesh;
	Vector2[] meshUvs;
	...
	{
		blaze2dSpriteGeomGen.SetSpriteColors (dest, 0, 16, _color, collectionInst.premultipliedAlpha);
	}
	...
	protected void SetGeometry(Vector3[] vertices, Vector2[] uvs)
	{
		...
		blaze2dSpriteGeomGen.SetSlicedSpriteGeom(meshVertices, meshUvs, 0, out boundsCenter, out boundsExtents, sprite, _scale, dimensions, new Vector2(borderLeft, borderBottom), new Vector2(borderRight, borderTop), anchor, colliderOffsetZ, colliderExtentZ);

		if (meshNormals.Length > 0 || meshTangents.Length > 0) {
			blaze2dSpriteGeomGen.SetSpriteVertexNormals(meshVertices, meshVertices[0], meshVertices[15], sprite.normals, sprite.tangents, meshNormals, meshTangents);
		}
	...
	void SetIndices()
	{
		...
		blaze2dSpriteGeomGen.SetSlicedSpriteIndices(meshIndices, 0, 0, CurrentSprite, _borderOnly);
	}
	...
	void PermanentUpgradeLegacyMode() {
		blaze2dSpriteDefinition def = CurrentSprite;
		...
		if 		(NearEnough(x,    0, w) && NearEnough(y, -h/2, h))	_anchor = blaze2dBaseSprite.Anchor.UpperCenter;
		else if (NearEnough(x,    0, w) && NearEnough(y,    0, h)) 	_anchor = blaze2dBaseSprite.Anchor.MiddleCenter;
		else if (NearEnough(x,    0, w) && NearEnough(y,  h/2, h)) 	_anchor = blaze2dBaseSprite.Anchor.LowerCenter;
		else if (NearEnough(x, -w/2, w) && NearEnough(y, -h/2, h)) 	_anchor = blaze2dBaseSprite.Anchor.UpperRight;
		else if (NearEnough(x, -w/2, w) && NearEnough(y,    0, h)) 	_anchor = blaze2dBaseSprite.Anchor.MiddleRight;
		else if (NearEnough(x, -w/2, w) && NearEnough(y,  h/2, h)) 	_anchor = blaze2dBaseSprite.Anchor.LowerRight;
		else if (NearEnough(x,  w/2, w) && NearEnough(y, -h/2, h)) 	_anchor = blaze2dBaseSprite.Anchor.UpperLeft;
		else if (NearEnough(x,  w/2, w) && NearEnough(y,    0, h)) 	_anchor = blaze2dBaseSprite.Anchor.MiddleLeft;
		else if (NearEnough(x,  w/2, w) && NearEnough(y,  h/2, h)) 	_anchor = blaze2dBaseSprite.Anchor.LowerLeft;
		else {
			...
			_anchor = blaze2dBaseSprite.Anchor.MiddleCenter;
		}
