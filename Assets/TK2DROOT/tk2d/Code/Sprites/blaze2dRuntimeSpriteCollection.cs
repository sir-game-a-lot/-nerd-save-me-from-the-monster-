...
namespace tk2dRuntime
{
	static class SpriteCollectionGenerator 
	{
			...
			bool[] rotated)
		{
			...
			// Generate geometry
			sc.spriteDefinitions = new blaze2dSpriteDefinition[regions.Length];
		...
		static blaze2dSpriteDefinition CreateDefinitionForRegionInTexture(string name, Vector2 textureDimensions, float scale, Rect uvRegion, Rect trimRect, Vector2 anchor, bool rotated)
		{
			...
			var def = new blaze2dSpriteDefinition();
			def.flipped = rotated ? blaze2dSpriteDefinition.FlipMode.TPackerCW : blaze2dSpriteDefinition.FlipMode.None;
