...
[System.Serializable]
public class blaze2dBatchedSprite
{
	public enum Type {
		EmptyGameObject,
		Sprite,
	...
	public blaze2dSpriteCollectionData spriteCollection = null;
	...
	public blaze2dBaseSprite.Anchor anchor = blaze2dBaseSprite.Anchor.LowerLeft;
	...
	public blaze2dSpriteDefinition GetSpriteDefinition() {
...
/// </summary>
public class blaze2dStaticSpriteBatcher : MonoBehaviour, blaze2dRuntime.ISpriteCollectionForceBuild
{
	...
	/// </summary>
	public blaze2dBatchedSprite[] batchedSprites = null;
	public blaze2dTextMeshData[] allTextMeshData = null;
	public blaze2dSpriteCollectionData spriteCollection = null;
		...
		if (version < 3)
		{
			if (batchedSprites != null)
			{
				foreach (var sprite in batchedSprites)
				{
					...
					{
						sprite.type = blaze2dBatchedSprite.Type.EmptyGameObject;
					}
					else {
						sprite.type = blaze2dBatchedSprite.Type.Sprite;
	...
	public void UpdateMatrices() {
		...
		if (hasParentIds) {
			...
			List<blaze2dBatchedSprite> parentSortedSprites = new List<blaze2dBatchedSprite>( batchedSprites );
			parentSortedSprites.Sort((a, b) => a.parentId.CompareTo(b.parentId) );
			foreach (blaze2dBatchedSprite sprite in parentSortedSprites) {
			...
			foreach (blaze2dBatchedSprite sprite in batchedSprites) {
	...
	{
		List<blaze2dBatchedSprite> solidBatches = new List<blaze2dBatchedSprite>();
		List<blaze2dBatchedSprite> otherBatches = new List<blaze2dBatchedSprite>();
		List<blaze2dBatchedSprite> undrawnBatches = new List<blaze2dBatchedSprite>();
		foreach (blaze2dBatchedSprite batchedSprite in batchedSprites)
		{
		...
		List<blaze2dBatchedSprite> allBatches = new List<blaze2dBatchedSprite>(solidBatches.Count + otherBatches.Count + undrawnBatches.Count);
		allBatches.AddRange(solidBatches);
		allBatches.AddRange(otherBatches);
		allBatches.AddRange(undrawnBatches);
		
		// Re-index parents
		Dictionary<blaze2dBatchedSprite, int> lookup = new Dictionary<blaze2dBatchedSprite, int>();
		int index = 0;
	...
	Material GetMaterial(blaze2dBatchedSprite bs) {
		switch (bs.type) {
			case blaze2dBatchedSprite.Type.EmptyGameObject: return null;
			case blaze2dBatchedSprite.Type.TextMesh: return allTextMeshData[bs.xRefId].font.materialInst;
	...
	void BuildRenderMesh()
	{
		...
		foreach (var bs in batchedSprites)
		{
			...
			{
				blaze2dTextMeshData textMeshData = allTextMeshData[bs.xRefId];
		...
		foreach (var bs in batchedSprites) 
		{
			...
			case blaze2dBatchedSprite.Type.Sprite:
				if (spriteDef != null) blaze2dSpriteGeomGen.GetSpriteGeomDesc(out nVerts, out nInds, spriteDef);
				break;
			case blaze2dBatchedSprite.Type.TiledSprite:
				if (spriteDef != null) blaze2dSpriteGeomGen.GetTiledSpriteGeomDesc(out nVerts, out nInds, spriteDef, bs.Dimensions);
				break;
			case blaze2dBatchedSprite.Type.SlicedSprite:
				if (spriteDef != null) blaze2dSpriteGeomGen.GetSlicedSpriteGeomDesc(out nVerts, out nInds, spriteDef, bs.CheckFlag(blaze2dBatchedSprite.Flags.SlicedSprite_BorderOnly));
				break;
			case blaze2dBatchedSprite.Type.ClippedSprite:
				if (spriteDef != null) blaze2dSpriteGeomGen.GetClippedSpriteGeomDesc(out nVerts, out nInds, spriteDef);
				break;
			case blaze2dBatchedSprite.Type.TextMesh:
				{
					blaze2dTextMeshData textMeshData = allTextMeshData[bs.xRefId];
					blaze2dTextGeomGen.GetTextMeshGeomDesc(out nVerts, out nInds, blaze2dTextGeomGen.Data(textMeshData, textMeshData.font.inst, bs.FormattedText));
					break;
				}
			}
			numVertices += nVerts;
		...
		foreach (var bs in batchedSprites)
		{
			...
			if (bs.type == blaze2dBatchedSprite.Type.EmptyGameObject)
			{
			...
			case blaze2dBatchedSprite.Type.Sprite:
				if (spriteDef != null) {
					blaze2dSpriteGeomGen.SetSpriteGeom(posData, uvData, normalData, tangentData, 0, spriteDef, Vector3.one);
					blaze2dSpriteGeomGen.SetSpriteIndices(indData, 0, currVertex, spriteDef);
				}
				break;
			case blaze2dBatchedSprite.Type.TiledSprite:
				if (spriteDef != null) {
					blaze2dSpriteGeomGen.SetTiledSpriteGeom(posData, uvData, 0, out boundsCenter, out boundsExtents, spriteDef, Vector3.one, bs.Dimensions, bs.anchor, bs.BoxColliderOffsetZ, bs.BoxColliderExtentZ);
					blaze2dSpriteGeomGen.SetTiledSpriteIndices(indData, 0, currVertex, spriteDef, bs.Dimensions);
				}
				break;
			case blaze2dBatchedSprite.Type.SlicedSprite:
				if (spriteDef != null) {
					blaze2dSpriteGeomGen.SetSlicedSpriteGeom(posData, uvData, 0, out boundsCenter, out boundsExtents, spriteDef, Vector3.one, bs.Dimensions, bs.SlicedSpriteBorderBottomLeft, bs.SlicedSpriteBorderTopRight, bs.anchor, bs.BoxColliderOffsetZ, bs.BoxColliderExtentZ);
					blaze2dSpriteGeomGen.SetSlicedSpriteIndices(indData, 0, currVertex, spriteDef, bs.CheckFlag(blaze2dBatchedSprite.Flags.SlicedSprite_BorderOnly));
				}
				break;
			case blaze2dBatchedSprite.Type.ClippedSprite:
				if (spriteDef != null) {
					blaze2dSpriteGeomGen.SetClippedSpriteGeom(posData, uvData, 0, out boundsCenter, out boundsExtents, spriteDef, Vector3.one, bs.ClippedSpriteRegionBottomLeft, bs.ClippedSpriteRegionTopRight, bs.BoxColliderOffsetZ, bs.BoxColliderExtentZ);
					blaze2dSpriteGeomGen.SetClippedSpriteIndices(indData, 0, currVertex, spriteDef);
				}
				break;
			case blaze2dBatchedSprite.Type.TextMesh:
				{
					blaze2dTextMeshData textMeshData = allTextMeshData[bs.xRefId];
					var geomData = blaze2dTextGeomGen.Data(textMeshData, textMeshData.font.inst, bs.FormattedText);
					int target = blaze2dTextGeomGen.SetTextMeshGeom(posData, uvData, uv2Data, colorData, 0, geomData);
					...
					blaze2dTextGeomGen.SetTextMeshIndices(indData, 0, currVertex, geomData, target);
					break;
				}
			}
			...
			if (nVerts > 0 && bs.type != blaze2dBatchedSprite.Type.TextMesh)
			{
				bool premulAlpha = (bs.spriteCollection != null) ? bs.spriteCollection.premultipliedAlpha : false;
				blaze2dSpriteGeomGen.SetSpriteColors(colorData, 0, nVerts, bs.color, premulAlpha);
			}
	...
	void BuildPhysicsMesh()
	{
		...
		foreach (var bs in batchedSprites) 
		{
			...
			blaze2dSpriteDefinition spriteDef = bs.GetSpriteDefinition();
			...
			{
			case blaze2dBatchedSprite.Type.Sprite:
				if (spriteDef != null && spriteDef.colliderType == blaze2dSpriteDefinition.ColliderType.Mesh)
				{
					buildSpriteDefinitionMesh = true;
				}
				if (spriteDef != null && spriteDef.colliderType == blaze2dSpriteDefinition.ColliderType.Box)
				{
					buildBox = true;
				}
				break;
			...
			case blaze2dBatchedSprite.Type.SlicedSprite:
			case blaze2dBatchedSprite.Type.TiledSprite:
				buildBox = bs.CheckFlag(blaze2dBatchedSprite.Flags.Sprite_CreateBoxCollider);
				break;
			case blaze2dBatchedSprite.Type.TextMesh:
				//...
				break;
			...
			if (spriteDef.physicsEngine == blaze2dSpriteDefinition.PhysicsEngine.Physics2D) {
				physics3D = false;
			}
		}
	...
	void BuildPhysicsMesh2D(EdgeCollider2D[] edgeColliders, int numEdgeColliders) {
		...
		foreach (blaze2dBatchedSprite bs in batchedSprites) 
		{
			if (!bs.IsDrawn) // when the first non-drawn child is found, it signals the end of the drawn list
				break;
	...
	void BuildPhysicsMesh3D(MeshCollider meshCollider, bool flattenDepth, int numVertices, int numIndices) {
		...
		foreach (var bs in batchedSprites) 
		{
			...
			if (buildSpriteDefinitionMesh)
			{
				blaze2dSpriteGeomGen.SetSpriteDefinitionMeshData(vertices, indices, currVertex, currIndex, currVertex, spriteDef, mat, bs.baseScale);
				currVertex += spriteDef.colliderVertices.Length;
				currIndex += spriteDef.colliderIndicesFwd.Length;
			}
			else if (buildBox)
			{
				blaze2dSpriteGeomGen.SetBoxMeshData(vertices, indices, currVertex, currIndex, currVertex, boxOrigin, boxExtents, mat, bs.baseScale);
				currVertex += 8;
				currIndex += 6 * 6;
			}
		}
	...
	public bool UsesSpriteCollection(blaze2dSpriteCollectionData spriteCollection)
	{
