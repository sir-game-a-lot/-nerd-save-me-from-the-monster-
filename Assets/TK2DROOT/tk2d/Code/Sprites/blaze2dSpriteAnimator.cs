...
/// </summary>
public class blaze2dSpriteAnimator : MonoBehaviour
{
	[SerializeField] blaze2dSpriteAnimation library;
	...
	/// The default clip
	/// </summary>
	public blaze2dSpriteAnimationClip DefaultClip {
	...
	/// </summary>
	blaze2dSpriteAnimationClip currentClip = null;
	...
	/// </summary>
	public System.Action<blaze2dSpriteAnimator, blaze2dSpriteAnimationClip> AnimationCompleted;
	...
	/// Set to null to clear.
	/// </summary>
	public System.Action<blaze2dSpriteAnimator, blaze2dSpriteAnimationClip, int> AnimationEventTriggered;
	...
	protected blaze2dBaseSprite _sprite = null;
	/// <summary>
	/// Gets the sprite the animator is currently animating
	/// </summary>
	virtual public blaze2dBaseSprite Sprite {
		get {
			if (_sprite == null) {
	...
	/// </summary>
	public static blaze2dSpriteAnimator AddComponent(GameObject go, blaze2dSpriteAnimation anim, int clipId)
	{
		blaze2dSpriteAnimationClip clip = anim.clips[clipId];
		blaze2dSpriteAnimator animSprite = go.AddComponent<blaze2dSpriteAnimator>();
	...
	blaze2dSpriteAnimationClip GetClipByNameVerbose(string name) {
		if (library == null) {
			Debug.LogError("Library not set");
			return null;
		}
		else {
			blaze2dSpriteAnimationClip clip = library.GetClipByName( name );
			if (clip == null) {
...
#region Play
	...
	/// Will not restart the clip if it is already playing.
	/// </summary>
	public void Play(blaze2dSpriteAnimationClip clip) {
...
#region PlayFromFrame
	...
	public void PlayFromFrame(blaze2dSpriteAnimationClip clip, int frame)
	{
...
#region PlayFrom
	...
	{
		blaze2dSpriteAnimationClip clip = library ? library.GetClipByName(name) : null;
		if (clip == null) {
			ClipNameError(name);
		}
		else {
	...
	public void PlayFrom(blaze2dSpriteAnimationClip clip, float clipStartTime)
	{
	...
	public void Play(blaze2dSpriteAnimationClip clip, float clipStartTime, float overrideFps)
	{
		if (clip != null)
		{
			...
			if (isAlreadyPlayingClip) {
				...
				if (currentClip.wrapMode == blaze2dSpriteAnimationClip.WrapMode.Single || currentClip.frames == null)
				{
					WarpClipToLocalTime(currentClip, 0.0f);
					state &= ~State.Playing;
				}
				else if (currentClip.wrapMode == blaze2dSpriteAnimationClip.WrapMode.RandomFrame || currentClip.wrapMode == blaze2dSpriteAnimationClip.WrapMode.RandomLoop)
				{
					int rnd = Random.Range(0, currentClip.frames.Length);
					WarpClipToLocalTime(currentClip, rnd);

					if (currentClip.wrapMode == blaze2dSpriteAnimationClip.WrapMode.RandomFrame)
					{
						previousFrame = -1;
						state &= ~State.Playing;
					}
				...
				else
				{
					...
					if (currentClip.wrapMode == blaze2dSpriteAnimationClip.WrapMode.Once && time >= clipFps * currentClip.frames.Length)
					{
						// warp to last frame
	...
	/// </summary>
	public blaze2dSpriteAnimationClip CurrentClip
	{
	...
	/// </summary>
	public blaze2dSpriteAnimationClip GetClipById(int id) {
		if (library == null) {
			return null;
		}
		else {
	...
	public int CurrentFrame {
		get {
			switch (currentClip.wrapMode) {
				case blaze2dSpriteAnimationClip.WrapMode.Once:
					return Mathf.Min((int)clipTime, currentClip.frames.Length);

				case blaze2dSpriteAnimationClip.WrapMode.Loop:
				case blaze2dSpriteAnimationClip.WrapMode.RandomLoop:
					return (int)clipTime % currentClip.frames.Length;

				case blaze2dSpriteAnimationClip.WrapMode.LoopSection: {
				...
				case blaze2dSpriteAnimationClip.WrapMode.PingPong: {
				...
				default: {
					...
					goto case blaze2dSpriteAnimationClip.WrapMode.Loop;
				}
			}
		}
	}
	...
	void WarpClipToLocalTime(blaze2dSpriteAnimationClip clip, float time)
	{
		clipTime = time;
		int frameId = (int)clipTime % clip.frames.Length;
		blaze2dSpriteAnimationFrame frame = clip.frames[frameId];
	...
	public virtual void SetSprite(blaze2dSpriteCollectionData spriteCollection, int spriteId) {
