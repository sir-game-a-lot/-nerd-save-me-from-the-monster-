...
[AddComponentMenu("blaze2D/Camera/blaze2dCameraAnchor")]
[ExecuteInEditMode]
/// <summary>
/// Anchors children to anchor position, offset by number of pixels
/// </summary>
public class blaze2dCameraAnchor : MonoBehaviour 
{
	// Legacy anchor
	...
	[SerializeField]
	blaze2dBaseSprite.Anchor _anchorPoint = blaze2dBaseSprite.Anchor.UpperLeft;
	...
	/// </summary>
	public blaze2dBaseSprite.Anchor AnchorPoint {
		get {
			if (anchor != -1) {
				if (anchor >= 0 && anchor <= 2) _anchorPoint = (blaze2dBaseSprite.Anchor)( anchor + 6 );
				else if (anchor >= 6 && anchor <= 8) _anchorPoint = (blaze2dBaseSprite.Anchor)( anchor - 6 );
				else _anchorPoint = (blaze2dBaseSprite.Anchor)( anchor );
				anchor = -1;
#if UNITY_EDITOR
	...
	blaze2dCamera _anchorTk2dCamera = null;
	...
	blaze2dCamera AnchorTk2dCamera {
		get {
			if (_anchorCameraCached != _anchorCamera) {
				_anchorTk2dCamera = _anchorCamera.GetComponent<blaze2dCamera>();
	...
	void UpdateTransform()
	{
		...
		blaze2dCamera currentCamera = (AnchorTk2dCamera != null && AnchorTk2dCamera.CameraSettings.projection != blaze2dCameraSettings.ProjectionType.Perspective) ? AnchorTk2dCamera : null;
		...
		switch (AnchorPoint)
		{
		case blaze2dBaseSprite.Anchor.UpperLeft: 		anchoredPosition = new Vector3(x_lhs, y_top, position.z); break;
		case blaze2dBaseSprite.Anchor.UpperCenter: 	anchoredPosition = new Vector3(x_ctr, y_top, position.z); break;
		case blaze2dBaseSprite.Anchor.UpperRight: 		anchoredPosition = new Vector3(x_rhs, y_top, position.z); break;
		case blaze2dBaseSprite.Anchor.MiddleLeft: 		anchoredPosition = new Vector3(x_lhs, y_ctr, position.z); break;
		case blaze2dBaseSprite.Anchor.MiddleCenter:	anchoredPosition = new Vector3(x_ctr, y_ctr, position.z); break;
		case blaze2dBaseSprite.Anchor.MiddleRight: 	anchoredPosition = new Vector3(x_rhs, y_ctr, position.z); break;
		case blaze2dBaseSprite.Anchor.LowerLeft: 		anchoredPosition = new Vector3(x_lhs, y_bot, position.z); break;
		case blaze2dBaseSprite.Anchor.LowerCenter: 	anchoredPosition = new Vector3(x_ctr, y_bot, position.z); break;
		case blaze2dBaseSprite.Anchor.LowerRight: 		anchoredPosition = new Vector3(x_rhs, y_bot, position.z); break;
		}
