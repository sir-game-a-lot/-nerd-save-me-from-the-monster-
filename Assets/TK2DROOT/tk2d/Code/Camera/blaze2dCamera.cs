...
[AddComponentMenu("blaze2D/Camera/blaze2dCamera")]
[ExecuteInEditMode]
/// <summary>
...
/// </summary>
public class blaze2dCamera : MonoBehaviour 
{
	static int CURRENT_VERSION = 1;
	public int version = 0;

	[SerializeField] private blaze2dCameraSettings cameraSettings = new blaze2dCameraSettings();

	/// <summary>
	/// The unity camera settings. 
	/// Use this instead of camera.XXX to change parameters.
	/// </summary>
	public blaze2dCameraSettings CameraSettings {
		get {
	...
	/// </summary>
	public blaze2dCameraResolutionOverride[] resolutionOverride = new blaze2dCameraResolutionOverride[1] {
		blaze2dCameraResolutionOverride.DefaultOverride
	};

	/// <summary>
	/// The currently used override
	/// </summary>
	public blaze2dCameraResolutionOverride CurrentResolutionOverride {
		get { 
			blaze2dCamera settings = SettingsRoot;
			...
			blaze2dCameraResolutionOverride currentResolutionOverride = null;
	...
	/// </summary>
	public blaze2dCamera InheritConfig {
	...
	[SerializeField]
	private blaze2dCamera inheritSettings = null;
	...
	private static List<blaze2dCamera> allCameras = new List<blaze2dCamera>();

	/// <summary>
	/// Returns the first camera in the list that can "see" this layer, or null if none can be found
	/// </summary>
	public static blaze2dCamera CameraForLayer( int layer ) {
		int layerMask = 1 << layer;
		int cameraCount = allCameras.Count;
		for (int i = 0; i < cameraCount; ++i) {
			blaze2dCamera cam = allCameras[i];
	...
	void Awake () {
		...
		blaze2dCamera settings = SettingsRoot;
		blaze2dCameraSettings inheritedCameraSettings = settings.CameraSettings;
		if (inheritedCameraSettings.projection == blaze2dCameraSettings.ProjectionType.Perspective) {
	...
	void OnPreCull() {
		...
		blaze2dUpdateManager.FlushQueues();
		UpdateCameraMatrix();
	}
	...
	public float GetSizeAtDistance(float distance) {
		blaze2dCameraSettings cameraSettings = SettingsRoot.CameraSettings;
		switch (cameraSettings.projection) {
			case blaze2dCameraSettings.ProjectionType.Orthographic:
				if (cameraSettings.orthographicType == blaze2dCameraSettings.OrthographicType.PixelsPerMeter) {
			...
			case blaze2dCameraSettings.ProjectionType.Perspective:
	...
	blaze2dCamera _settingsRoot;
	public blaze2dCamera SettingsRoot {
		get { 
...
#if UNITY_EDITOR
	public static blaze2dCamera Editor__Inst {
		get {
			if (inst != null) {
				return inst;
			}
			return GameObject.FindObjectOfType(typeof(blaze2dCamera)) as blaze2dCamera;
		}
	}
#endif
	...
	Vector2 GetScaleForOverride(blaze2dCamera settings, blaze2dCameraResolutionOverride currentOverride, float width, float height) {
		...
		{
		case blaze2dCameraResolutionOverride.AutoScaleMode.PixelPerfect:
			s = 1;
			scale.Set(s, s);
			break;

		case blaze2dCameraResolutionOverride.AutoScaleMode.FitHeight: 
			s = height / settings.nativeResolutionHeight; 
			scale.Set(s, s);
			break;

		case blaze2dCameraResolutionOverride.AutoScaleMode.FitWidth: 
			s = width / settings.nativeResolutionWidth; 
			scale.Set(s, s);
			break;

		case blaze2dCameraResolutionOverride.AutoScaleMode.FitVisible:
		case blaze2dCameraResolutionOverride.AutoScaleMode.ClosestMultipleOfTwo:
			...
			if (currentOverride.autoScaleMode == blaze2dCameraResolutionOverride.AutoScaleMode.ClosestMultipleOfTwo)
			{
				if (s > 1.0f)
		...
		case blaze2dCameraResolutionOverride.AutoScaleMode.StretchToFit:
			scale.Set(width / settings.nativeResolutionWidth, height / settings.nativeResolutionHeight);
			break;

		case blaze2dCameraResolutionOverride.AutoScaleMode.Fill:
			s = Mathf.Max(width / settings.nativeResolutionWidth,height / settings.nativeResolutionHeight);
			scale.Set(s, s);
			break;

		default:
		case blaze2dCameraResolutionOverride.AutoScaleMode.None: 
	...
	Vector2 GetOffsetForOverride(blaze2dCamera settings, blaze2dCameraResolutionOverride currentOverride, Vector2 scale, float width, float height) {
		...
		switch (currentOverride.fitMode) {
			case blaze2dCameraResolutionOverride.FitMode.Center:
				if (settings.cameraSettings.orthographicOrigin == blaze2dCameraSettings.OrthographicOrigin.BottomLeft) {
			...
			default:
			case blaze2dCameraResolutionOverride.FitMode.Constant: 
	...
	public Matrix4x4 Editor__GetNativeProjectionMatrix(  ) {
		blaze2dCamera settings = SettingsRoot;
		if (settings.CameraSettings.projection == blaze2dCameraSettings.ProjectionType.Perspective) {
	...
	Matrix4x4 GetProjectionMatrixForOverride( blaze2dCamera settings, blaze2dCameraResolutionOverride currentOverride, float pixelWidth, float pixelHeight, bool halfTexelOffset, out Rect screenExtents, out Rect unscaledScreenExtents ) {
		...
		if (this.viewportClippingEnabled && this.InheritConfig != null) {
			...
			if (settings.cameraSettings.orthographicOrigin == blaze2dCameraSettings.OrthographicOrigin.Center) {
	...
	Vector2 GetScreenPixelDimensions(blaze2dCamera settings) {
	...
	private void Upgrade() {
		if (version != CURRENT_VERSION) {
			if (version == 0) {
				...
				cameraSettings.orthographicType = blaze2dCameraSettings.OrthographicType.PixelsPerMeter;
				cameraSettings.orthographicOrigin = blaze2dCameraSettings.OrthographicOrigin.BottomLeft;
				cameraSettings.projection = blaze2dCameraSettings.ProjectionType.Orthographic;

				foreach (blaze2dCameraResolutionOverride ovr in resolutionOverride) {
					ovr.Upgrade( version );
				}
				...
				if (unityCamera != null) {
					...
					if (!unityCamera.isOrthoGraphic) {
						cameraSettings.projection = blaze2dCameraSettings.ProjectionType.Perspective;
